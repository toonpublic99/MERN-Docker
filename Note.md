1. docker-compose config (ตรวจสอบความถูกต้องของ config)
1.1 docker-compose config -q (ตรวจสอบความถูกต้องของ config โดยถ้ามี error จะมีการแจ้ง ถ้าไม่มีก็จะไม่แจ้ง)
2. docker-compose up -d (สร้าง images และ containers)
3. docker-compose ps (ตรวจสอบ service ที่รับผิดชอบ)
4. docker images (ตรวจสอบ image)
